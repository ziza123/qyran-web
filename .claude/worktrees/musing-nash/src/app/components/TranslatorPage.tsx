...
export function TranslatorPage({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const ground = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x1E293B, roughness: 0.8, transparent: true, opacity: 0.5 })
    );
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white flex flex-col" style={{ fontFamily: '"Space Grotesk", sans-serif' }}>
      ...
      <div className="flex-1 flex flex-col max-w-[480px] w-full mx-auto p-3 gap-3">
        ...
        {/* Status Bar */}
        <div className={`rounded-xl border border-[#334155] p-3.5 text-center ${isPlaying ? 'bg-[#1E293B]/50' : 'bg-[#1E293B]/30'}`}>
          <span className={`text-sm ${isPlaying ? 'text-[#A5B4FC] text-xl font-bold tracking-widest' : 'text-[#64748B]'}`}>
            {isPlaying ? currentSign : statusText}
          </span>
          {isPlaying && (
            <div className="h-1 bg-white/10 rounded-sm mt-2.5 overflow-hidden">
              <div className="h-full bg-[#4F46E5] rounded-sm transition-[width] duration-150" style={{ width: `${progress}%` }} />
            </div>
          )}
        </div>
              ...
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                  speed === s
                    ...
                    : 'border-[#334155] text-[#64748B] hover:border-[#4F46E5] hover:text-[#4F46E5]'
                }`}
              >
                {s}x
              </button>
        ...
        <div className="flex flex-col gap-2.5">
          <textarea
            ...
            className={`w-full bg-[#1E293B]/50 border rounded-xl p-3.5 text-white placeholder-[#64748B] resize-none font-[inherit] outline-none transition-colors ${
              isListening ? 'border-red-500' : 'border-[#334155] focus:border-[#4F46E5]'
            }`}
          />
          <div className="flex gap-2.5">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl bg-[#4F46E5] text-white font-semibold transition-opacity disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90"
            >
            ...
            <button
              ...
              className={`w-13 h-13 flex items-center justify-center rounded-xl border text-xl transition-all ${
                isListening
                  ...
                  : 'bg-[#1E293B]/50 border-[#334155] hover:border-[#4F46E5]'
              }`}
            >
